...
d3.json(`/dates`).then(data => {
    ...
    function loadPage(date) {
        ...
        // Call Route
        d3.json(`/positive_increase/${date}`).then(data => {
            ...
            for (var i = 0; i < data.length; i++) {
                bar_states.push(data[i][1])
            }

            // Empty Array to Hold Values
            var bar_positive_increase = []

            // For Loop to Push Values to Array
            for (var i = 0; i < data.length; i++) {
                bar_positive_increase.push(data[i][0])
            }

            // Empty Array to Hold Color
            var bar_color = []

            // For Loop to Push Color to Array
            for (var i = 0; i < data.length; i++) {
                if (data[i][2] === "R") { bar_color.push("red") } else if (data[i][2] === "D") { bar_color.push("blue") }
            }

            // Create Bar Data
            var barData = [{
                x: bar_positive_increase,
                y: bar_states,
                transforms: [{
                    type: 'sort',
                    target: 'x',
