...
d3.json(`/dates`).then(data => {
    ...
    function loadPage(date) {
        ...
        // Call Route
        d3.json(`/negative_increase/${date}`).then(data => {
            ...
            var bar_negative_increase = []

            // For Loop to Push Values to Array
            for (var i = 0; i < data.length; i++) {
                bar_negative_increase.push(data[i][0])
            }
